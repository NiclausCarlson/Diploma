...
-- 
-- private def OrdType {ord: Type} [MonomialOrder $ Variables Dimension ord]: Parsec (ord × String) := do
--   let lex   := "lex"
--   let grlex := "grlex"
--   ws *> skipChar '[' 
--   let name ← ws *> (pstring lex <|> pstring grlex) 
--   ws *> skipChar ']' 
--   if name == lex then return (order.Lex, lex)
--   else if name == grlex then return (order.GrLex, grlex)
